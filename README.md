...
- **检测**: 用户消息经独立预警分级 (NONE / YELLOW / RED), 不依赖对话模型的自觉
- **在线通道**: RED 时 WebSocket 实时弹窗, 直达心理援助热线
- **机构通道**: Webhook 同步推送机构服务端 (fire-and-forget, 3s 超时, 永不阻塞主链路), 两通道互为冗余
...
| 预警通知渠道   | `IAlertNotifier` 端口, 短信/IM 渠道同构接入   |
| 心理知识包     | classpath 整包替换, 缺失自动回退内置包        |
| 提示词体系     | 三条系统提示词均可整体覆盖                    |
| 身份品牌       | 品牌名一行配置, 面向不同机构白标              |

另有**结构化输出预埋 ("副医生")**: 开关开启后 AI 在共情回复之外产出结构化心理标签 (供未来咨询员工作台消费), 后端拆流, 前端零感知; 结构定义支持机构以自然语言描述, 启动时经 LLM 归一化为严格 JSON Schema 并按指纹缓存.
...
**企业就绪细节**: JWT 认证 (HS256, 黑名单登出), PBKDF2WithHmacSHA256 密码哈希 (210k 轮, OWASP 推荐), 三级限流 (聊天/登录/语音), CORS 白名单, 健康探针 (`/q/health/live` / `ready`), 12-factor 环境变量配置 (39 项 `SOULNOTES_*` 键).
...
| 质量门禁     | 466 个测试 (含 Mock-LLM 全链路集成), GitHub Actions 四工作流全自动化   |
...
| 文档                                          | 受众与内容                                       |
| --------------------------------------------- | ------------------------------------------------ |
| [CONFIGURATION.md](./CONFIGURATION.md)        | 部署者/运维者 — 环境变量总表, 配置向导, 部署形态, 机构集成 (Webhook / 知识包 / 提示词 / 副医生) |
| [Architecture.md](./Architecture.md)          | 开发者 — 包结构, 安全设计, 响应式约束, AI 链路, 数据流全景 |

## 路线图

- 咨询员工作台: 消费"副医生"结构化标签 (产出管线已就绪, 存储与消费端待建)
- 心理知识包 RAG 化: 向量检索升级 (当前为关键词检索, 接口已预留)
- 更多预警渠道: 短信 / IM (与 `IAlertNotifier` 同构接入)
- 更多 ASR 引擎选项 (引擎端口已抽象)
