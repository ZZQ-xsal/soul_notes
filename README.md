...
**企业就绪细节**: JWT 认证 (HS256, 黑名单登出), PBKDF2WithHmacSHA256 密码哈希 (210k 轮, OWASP 推荐), 三级限流 (聊天/登录/语音), CORS 白名单, 健康探针 (`/q/health/live` / `ready`), 12-factor 环境变量配置 (42 项 `SOULNOTES_*` 键).
...
| 质量门禁     | 486 个测试 (含 Mock-LLM 全链路集成), GitHub Actions 四工作流全自动化   |
