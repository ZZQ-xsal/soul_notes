...
export default function DiaryView() {
  ...
  const [draft, setDraft] = useState<DateFilter>({ startDate: '', endDate: '' })
  ...
  const handleCreated = (diary: DiaryItem): void => {
    ...
    const inFilter =
      (!filter.startDate || createdAt >= new Date(filter.startDate + 'T00:00:00')) &&
      ...
      <div className="view-head">
        <div>
          <h1 className="view-title">
            心情日记
            {items.length > 0 && <span className="view-count tabular"> (本页 {items.length} 条)</span>}
          </h1>
          <p className="view-sub">记录此刻, 让情绪被看见</p>
        </div>
        <div className="view-actions">
          <button type="button" className="btn secondary" onClick={() => void load(page)} disabled={loading}>
            刷新
          </button>
          <button type="button" className="btn" onClick={() => setEditorOpen(true)}>
            ✏ 写日记
          </button>
        </div>
      </div>
      ...
      <div className={`diary-list${loading ? ' is-loading-frame' : ''}`}>
        {items.length === 0 && !loading ? (
          <div className="empty-state card">
            ...
            <p>还没有日记, 写下第一篇吧</p>
            <button type="button" className="btn secondary" onClick={() => setEditorOpen(true)}>
              写日记
            </button>
          </div>
        ) : (
          items.map((d) => <DiaryCard key={d.id} diary={d} onOpen={setDetail} onDelete={(diary) => void handleDelete(diary)} />)
        )}
      </div>

      {items.length > 0 && (
        <div className="pager">
          <button type="button" className="btn secondary sm" disabled={page <= 1 || loading} onClick={() => setPage(page - 1)}>
            上一页
          </button>
          <span className="pager-info tabular">第 {page} 页</span>
          <button type="button" className="btn secondary sm" disabled={!hasMoreRef.current || loading} onClick={() => setPage(page + 1)}>
            下一页
          </button>
        </div>
      )}

      {editorOpen && <DiaryEditor onClose={() => setEditorOpen(false)} onCreated={handleCreated} />}
      {detail && <DiaryDetailModal diary={detail} onClose={() => setDetail(null)} />}
    </div>
  )
}
