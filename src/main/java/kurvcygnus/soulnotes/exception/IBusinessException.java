...
/**
 * 业务异常门面接口, 为业务层提供声明式异常创建能力.
 * <p>通过静态工厂 {@code of(...)} 统一创建 {@link HolderException} / {@link DataHolderException} 实例,
 * 两者均为包级私有, 外部代码无需直接依赖具体实现类.</p>
 * <ul>
 *     <li>ErrorCode ≠ Tag — {@link ErrorCode} 用于前后端协议 (HTTP 状态码 + 业务码),
 *         {@link #tag()} 用于运维分类 (格式 {@code WHERE_WHAT_ACTION}, 如 {@code "AUTH_LOGIN_USER_NOT_FOUND"}).
 *         禁止将 {@code ErrorCode.name()} 直接作为 tag 传入, 实现类构造守卫会拒绝该形态.</li>
 *     <li>Factory 语义 — 工厂方法的 {@code factory} 参数用于将 message 转换为被包装的 {@link Throwable},
...
public sealed interface IBusinessException<T extends StructuredException & IBusinessException<T>> extends IDetailedThrowable<T, Object>
{
    ...
    /**
     * 创建携带额外上下文数据的业务异常.
     * <p>{@code data} 用于附加失败现场的领域对象 (如非法输入的实体、缺失记录的 ID),
     * 通过 {@link IDetailedThrowable#causeData()} 暴露给上层恢复逻辑.</p>
     *
     ...
     * @param data      附加失败上下文数据, 不得为 {@link ErrorCode} 实例
     * @return 业务异常实例, 永不为 {@code null}
...
/**
 * 不携带额外数据的业务异常实现.
 * <p>包级私有, 外部代码通过 {@link IBusinessException#of(ErrorCode, String, Function, String)} 间接创建.</p>
 * <ul>
 *     <li>构造守卫: 禁止 {@code tag} 等于 {@code errorCode.name()} — 避免 tag 沦为 ErrorCode 的别名.</li>
...
/**
 * 携带额外上下文数据的业务异常实现.
 * <p>包级私有, 外部代码通过 {@link IBusinessException#of(ErrorCode, String, Function, String, Object)} 间接创建.</p>
 * <ul>
 *     <li>构造守卫: 禁止 {@code tag} 等于 {@code errorCode.name()} — 避免 tag 沦为 ErrorCode 的别名.</li>
 *     <li>构造守卫: 禁止将 {@link ErrorCode} 实例作为 {@code data} 传入 — 防止敏感枚举泄漏到响应负载.</li>
 * </ul>
 */
